---
title: Per-tool
layout: default
nav_order: 3
has_children: true
permalink: /docs/per-tool/
---

# Per-tool

Practical references for the specific context artifacts in use today. Each page covers what the file is, what to put in it, what to leave out, and how to keep it from going stale.

These pages assume you've read [What is a context resource?](/docs/foundations/what-is-a-context-resource/) and [Context is a budget](/docs/foundations/context-is-a-budget/) — the recommendations here lean on those ideas.

## In this section

- **[Crafting a CLAUDE.md](/docs/per-tool/claude-md/)** — the file Claude Code reads first: what to put in, what to keep out, how to keep it from going stale.
- **[AGENTS.md and the convergence](/docs/per-tool/agents-md/)** — the shared file an increasing number of tools now read, and what that means for per-tool fragmentation.
